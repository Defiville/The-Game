...
describe('Deploy and test RadioTips', () => {
    ...
    it('Allow to tip existent artist with ERC20 by anyone but redeem it only by artist', async()=> {
        ...
        await expect(radioTips.connect(alice).redeemArtistTips(0, [isla.address], [islaToTip])).to.be.revertedWith('Only recipient')
        await expect(radioTips.connect(rehab).redeemArtistTips(0, [isla.address], [islaToTip])).to.be.revertedWith('Amount exceed tips')
    })
    ...
    it('Allow to tip existent artist with ETH by anyone but redeem it only by artist', async()=> {
        ...
        await expect(radioTips.connect(alice).redeemArtistTips(0, [ETH], [islaToTip])).to.be.revertedWith('Only recipient')
        await expect(radioTips.connect(rehab).redeemArtistTips(0, [ETH], [islaToTip])).to.be.revertedWith('Amount exceed tips')
    })
